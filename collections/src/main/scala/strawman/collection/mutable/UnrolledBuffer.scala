    ...
    with Serializable
{
  ...
  protected[this] def fromSpecificIterable(coll: strawman.collection.Iterable[T]) = iterableFactory.from(coll)
  protected[this] def newSpecificBuilder(): Builder[T, UnrolledBuffer[T]] = new UnrolledBuffer[T]

  def iterableFactory: SeqFactory[UnrolledBuffer] = UnrolledBuffer
...
object UnrolledBuffer extends StrictOptimizedSeqFactory[UnrolledBuffer] {

  def empty[A]: UnrolledBuffer[A] = empty(ClassTag.Any.asInstanceOf[ClassTag[A]])

  def empty[A](implicit t: ClassTag[A]): UnrolledBuffer[A] = new UnrolledBuffer[A]

  def from[A](source: strawman.collection.IterableOnce[A]): UnrolledBuffer[A] = newBuilder[A]().addAll(source)

  def from[A](source: strawman.collection.IterableOnce[A])(implicit t: ClassTag[A]): UnrolledBuffer[A] = newBuilder[A](t).addAll(source)

  def newBuilder[A](): UnrolledBuffer[A] = newBuilder[A](ClassTag.Any.asInstanceOf[ClassTag[A]])

  def newBuilder[A](implicit t: ClassTag[A]): UnrolledBuffer[A] = new UnrolledBuffer[A]
